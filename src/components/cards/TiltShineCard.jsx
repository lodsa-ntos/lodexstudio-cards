...
function TiltShineCard({
    ...
    const apply = () => {
        ...
        el.style.transform = `perspective(1100px) rotateX(${s.rx.toFixed(2)}deg) rotateY(${s.ry.toFixed(2)}deg) scale3d(1.02,1.02,1.02)`;
        if (Math.abs(s.rx - s.targetRx) > 0.01 || Math.abs(s.ry - s.targetRy) > 0.01) {
    ...
    const handleKey = (e) => {
        ...
        else if (e.key === "ArrowDown") targetRx = Math.min(maxTilt, targetRx + step);
        else if (e.key === "ArrowLeft") targetRy = Math.max(-maxTilt, targetRy - step);
        else if (e.key === "ArrowRight") targetRy = Math.min(maxTilt, targetRy + step);
        else if (e.key === "Escape") { targetRx = 0; targetRy = 0; }
        else return;
        e.preventDefault();
        ...
        <div className={`grid place-items-center ${className}`}>
            ...
            >
                ...
                >
                    ...
                    {/* Contrast / vignette */}
                    <div className="absolute inset-0 bg-[radial-gradient(circle_at_10%_15%,rgba(0,0,0,0)_0%,rgba(0,0,0,0.55)_95%)]" aria-hidden="true" />

                    {/* Soft border + focus aid */}
                    <div className="absolute inset-0 ring-1 ring-white/15 rounded-2xl pointer-events-none" aria-hidden="true" />
